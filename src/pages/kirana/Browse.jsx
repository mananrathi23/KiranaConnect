...
export default function Browse() {
  ...
  const [radius,   setRadius]   = useState(10000); // metres; null = no filter

  // Check whether the logged-in user has location saved in localStorage
  const hasLocation = useMemo(() => {
    const stored = JSON.parse(localStorage.getItem('kc_user') || '{}');
    const c = stored?.location?.coordinates;
    return c && (c[0] !== 0 || c[1] !== 0);
  }, []);

  const load = useCallback(async (r) => {
    setLoading(true); setError('');
    const effectiveRadius = r ?? 999_999_999;
    try { setProducts(await productsApi.getProductsWithRadius(effectiveRadius)); }
    catch (err) { setError(err.message); }
    finally { setLoading(false); }
  }, []); // no deps — radius is passed as arg, not captured from closure

  useEffect(() => { load(radius); }, [radius, load]); // re-fetch whenever radius changes
  ...
  if (error)   return <div className="page-container"><ErrorMessage message={error} onRetry={() => load(radius)} /></div>;
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            ...
            {hasLocation && radius && (
              <> &middot; within <strong>{fmtDist(radius)}</strong> of you</>
            )}
            {' · '}
      ...
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        {RADIUS_OPTIONS.map(opt => (
          <button
            ...
            style={{
              ...
              background: radius === opt.value ? 'var(--primary-btn)' : 'var(--surface)',
              color: radius === opt.value ? 'white' : 'var(--text-2)',
              fontSize: 13, fontWeight: radius === opt.value ? 700 : 500,
        ...
        {!hasLocation && (
          <span style={{
            ...
            border: '1px solid #D97706', padding: '3px 10px',
            borderRadius: 'var(--radius-full)', fontWeight: 600,
          }}>
            ⚠️ Log out &amp; back in to activate distance filtering
          </span>
        )}
      </div>
            ...
            return (
              <div key={product._id} className="card" style={{ overflow: 'hidden', transition: 'all 0.2s', position: 'relative' }}
                ...
                <div style={{ height: 90, background: `linear-gradient(135deg, ${cat.bg}, ${cat.bg}88)`,
                  ...
                  {/* Distance badge — top-left corner of image area */}
                  {product.wholesalerDistance != null && (
                    <span style={{
                      ...
                      📍 {fmtDist(product.wholesalerDistance)}
                    </span>
                  )}
                </div>
