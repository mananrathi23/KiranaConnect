...
export default function Cart() {
  ...
  const [paymentMode, setPaymentMode] = useState('PAID'); // 'PAID' | 'KHATA'
  const [khataResult, setKhataResult] = useState(null);  // last khata response
  ...
  const handlePlaceOrder = async () => {
    setPlacing(true); setError(''); setKhataResult(null);
    try {
      if (paymentMode === 'KHATA') {
        // Pay Later — goes through Khata controller
        const res = await khataApi.placeKhataOrder({
          items: cartItems.map(item => ({ product: item.productId, quantity: item.qty })),
        });
        setKhataResult(res);
        clearCart();
        navigate('/shop/order-confirm', { state: { orders: [res.order], khata: res.khata, paymentMode: 'KHATA' } });
      } else {
        // Pay Now — existing flow
        const orders = await ordersApi.placeOrder({
          items: cartItems.map(item => ({ product: item.productId, quantity: item.qty })),
        });
        clearCart();
        navigate('/shop/order-confirm', { state: { orders, paymentMode: 'PAID' } });
      }
    } catch (err) {
          ...
          <div className="card" style={{ padding: 24, position: 'sticky', top: 80 }}>
            ...
            <div style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)',
                ...
                fontFamily: 'Sora' }}>Payment Mode</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <button
                  onClick={() => setPaymentMode('PAID')}
                  style={{
                    padding: '10px 8px', borderRadius: 'var(--radius)',
                    border: paymentMode === 'PAID' ? '2px solid var(--green)' : '1.5px solid var(--border)',
                    background: paymentMode === 'PAID' ? 'var(--green-soft)' : 'var(--surface-3)',
                    color: paymentMode === 'PAID' ? 'var(--green)' : 'var(--text-2)',
                ...
                <button
                  ...
                  style={{
                    padding: '10px 8px', borderRadius: 'var(--radius)',
                    border: paymentMode === 'KHATA' ? '2px solid var(--primary-btn)' : '1.5px solid var(--border)',
                    background: paymentMode === 'KHATA' ? 'var(--primary-soft)' : 'var(--surface-3)',
                    color: paymentMode === 'KHATA' ? 'var(--primary-btn)' : 'var(--text-2)',
                    fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'all 0.2s',
                  }}
                >📒 Pay on Khata</button>
              </div>
              {paymentMode === 'KHATA' && (
                <div style={{ marginTop: 10, padding: '10px 12px',
                  ...
                  fontSize: 12, color: 'var(--primary-btn)', fontWeight: 500 }}>
                  📒 This order will be added to your Khata (credit ledger).
                  You can settle it later from the <strong>My Khata</strong> page.
                </div>
              )}
            </div>
            ...
            <button className="btn btn-primary btn-lg" style={{
              width: '100%',
              background: paymentMode === 'KHATA'
                ? 'linear-gradient(135deg, var(--primary-btn), #7C3AED)'
                : undefined
              }}
