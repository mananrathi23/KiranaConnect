          ...
          <div className="form-group">
            ...
            <button
              ...
              style={{
                ...
                padding: '10px 16px',
                borderRadius: 'var(--radius)',
                border: geoStatus === 'done'
                  ? '1.5px solid var(--green)'
                  : '1.5px solid var(--border)',
                background: geoStatus === 'done' ? 'var(--green-soft)' : 'var(--surface-3)',
                color: geoStatus === 'done' ? 'var(--green)' : 'var(--text-2)',
                fontSize: 13, fontWeight: 600, cursor: 'pointer',
                width: '100%', transition: 'all 0.2s',
              }}
            >
              {geoStatus === 'loading' && '⏳ Detecting location...'}
              {geoStatus === 'done'    && `✅ Location captured (${Number(form.lat).toFixed(4)}, ${Number(form.lng).toFixed(4)})`}
              {geoStatus === 'error'  && '❌ Location failed — click to retry'}
              {geoStatus === 'idle'   && '📍 Use My Current Location'}
            </button>
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
              Used to show nearby wholesalers / shops to the other party.
            </p>
          </div>
